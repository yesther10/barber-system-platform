...
Multi-tenant barbershop SaaS: Next.js (App Router) monolith + in-repo worker, TS everywhere, Prisma/Postgres, Zod contracts shared with future native apps. REST API-first — logic lives in route handlers/service modules, never components. Resolves the two deferred choices: Pix provider and cancellation defaults.
...
| 4 | Slot conflicts | app lock vs exclusion | btree_gist EXCLUDE + transactional re-validation |
...
- `packages/db` — Prisma schema + migrations (custom SQL for exclusion) + seed.
...
Barber: id, barbershopId, userId, specialties[], bio?, active
...
Conflict prevention — migration SQL beside Appointment:
...
Booking tx: re-check slot → INSERT appointment + outbox atomically; exclusion violation → 409.
...
- **LGPD**: Supabase SP region; minimal PII; export → structured JSON ≤ 15 days; deletion → cancel future appointments, null PII, keep anonymized legal rows.
...
| Integration (Vitest + real Postgres) | booking tx + outbox atomicity; **concurrent double-booking — two parallel inserts, exactly one succeeds**; cross-tenant → 404; webhook duplicate → no change; outbox retry | Testcontainers; per-tenant fixtures |
...
Greenfield: `prisma migrate dev` in dependency order; btree_gist + exclusion ship with Appointment. Per-tenant payments flag; webhook disable → manual in-shop fallback; outbox prevents lost emails. No data migration.
